...
class AggregationZeropad(Function):
    ...
    def forward(ctx, input, weight, kernel_size, stride, padding, dilation):
        ...
        with torch.cuda.device_of(input):
            f = load_kernel('aggregation_zeropad_forward_kernel', _aggregation_zeropad_forward_kernel,
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input):
            if ctx.needs_input_grad[0]:
                ...
                n = grad_input.numel()
                opt['nthreads'] = n
                f = load_kernel('aggregation_zeropad_input_backward_kernel', _aggregation_zeropad_input_backward_kernel,
                                **opt)
                f(block=(CUDA_NUM_THREADS, 1, 1),
                  grid=(GET_BLOCKS(n), 1, 1),
            ...
            if ctx.needs_input_grad[1]:
                ...
                f = load_kernel('aggregation_zeropad_weight_backward_kernel',
                                _aggregation_zeropad_weight_backward_kernel, **opt)
                f(block=(CUDA_NUM_THREADS, 1, 1),
                  grid=(GET_BLOCKS(n), 1, 1),
...
def test_aggregation_zeropad():
    ...
    assert torch.autograd.gradcheck(
        partial(aggregation_zeropad, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation),
        (x, w))
