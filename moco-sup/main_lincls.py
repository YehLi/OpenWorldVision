...
parser.add_argument('--lr-decay', default=0.1, type=float, help='lr decay')
...
parser.add_argument('--eval-freq', default=10, type=int,
                    metavar='N', help='evaluate models frequency (default: 10)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    assert args.dataset in DATASET_CLASS.keys()
    num_classes = DATASET_CLASS[args.dataset]
    # create model
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        adjust_learning_rate(optimizer, epoch, args, args.cos)
...
def train(train_loader, model, criterion, optimizer, epoch, args, logger=None):
    ...
    progress = ProgressMeter(
        len(train_loader),
        [eta, losses, top1, top5, lr, batch_time, data_time],
...
def validate(val_loader, model, criterion, args, logger=None):
    batch_time = AverageMeter('time', ':6.3f')
    losses = AverageMeter('loss', ':.4e')
    top1 = AverageMeter('acc@1', ':6.2f')
    top5 = AverageMeter('acc@5', ':6.2f')
