...
def train(train_loader, model, criterion, optimizer, epoch, args, logger=None, scaler=None):
    ...
    ac1 = AverageMeter('ac1', ':6.8f')
    ac2 = AverageMeter('ac2', ':6.8f')
    progress_infos = [eta, losses, ac1, ac2, lr, batch_time, data_time]
    ...
    for i, (images, targets) in enumerate(train_loader):
        ...
        if args.use_fp16:
            with torch.cuda.amp.autocast():
                output, target = model(im_q=im_q, im_k=im_k, target=targets)
                # loss = criterion(output, target)
                output_scale = output - output.max(dim=1, keepdims = True)[0]
                logits = torch.exp(output_scale)
                denominator = (logits * (1 - target)).sum(dim=-1, keepdims = True)
                prob = torch.log( logits / (logits + denominator))
                loss = (prob * target).sum(dim=-1) / target.sum(dim=-1)
                loss = -1. *  loss.mean()
        else:
            output, target = model(im_q=im_q, im_k=im_k, target=targets)
            # output: N, C
            ...
            loss = (prob * target).sum(dim=-1) / target.sum(dim=-1)
            loss = -1. * loss.mean()
        pred_label = (output > 0).type(torch.long)
        target = target.type(torch.long)
        ac1_ = (pred_label == target).float().mean()
        ac2_ = ((pred_label == target) * target).sum() * 1. / target.sum()
        ac1.update(ac1_.item(), output.size(0))
        ac2.update(ac2_.item(), output.size(0))
