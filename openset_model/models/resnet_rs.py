...
class ResNetRS(nn.Module):
    ...
    def forward_features_prob(self, x):
        x = self.forward_features(x)
...
@register_model
def resnet_rs101L_g(pretrained=False, **kwargs):
    model_args = dict(block=Bottleneck, layers=[3, 4, 23, 3],  
    stem_type='deep', stem_width=64, base_width=64, cardinality=1,
    block_args=dict(attn_layer='se'), **kwargs)
...
@register_model
def resnet_rs101L_L(pretrained=False, **kwargs):
    model_args = dict(block=Bottleneck, layers=[3, 4, 23, 3],  
    stem_type='deep', stem_width=64, base_width=64, cardinality=1,
    block_args=dict(attn_layer='se'), **kwargs)
    return _create_resnet_rs('resnet_rs101_L', pretrained, **model_args)