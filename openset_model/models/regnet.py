...
model_cfgs = dict(
    ...
    regnety_120_L=_mcfg(w0=168, wa=73.36, wm=2.37, group_w=112, depth=19, se_ratio=0.25),
)
...
def _cfg2(url=''):
    return {
        ...
        'crop_pct': 0.888, 'interpolation': 'bicubic',
        'mean': IMAGENET_DEFAULT_MEAN, 'std': IMAGENET_DEFAULT_STD,
        'first_conv': 'stem.conv', 'classifier': 'head.fc',
    }
...
class RegNet(nn.Module):
    ...
    def forward_features_pool(self, x):
        ...
        if self.head.drop_rate:
            x = F.dropout(x, p=float(self.head.drop_rate), training=self.training)
        return x
...
@register_model
def regnety_120_g(pretrained=False, **kwargs):
    """RegNetY-12GF"""
    kwargs['global_pool'] = 'gavg'
    return _create_regnet('regnety_120', pretrained, **kwargs)

@register_model
def regnety_120_L(pretrained=False, **kwargs):
    """RegNetY-12GF"""
    return _create_regnet('regnety_120_L', pretrained, **kwargs)
