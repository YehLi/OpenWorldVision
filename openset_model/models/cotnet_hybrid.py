...
class CoTHybridNet(nn.Module):
    ...
    def forward_features_pool(self, x):
...
def cotnet50_hybrid_se(pretrained=False, **kwargs):
    ...
    stem_type='deep', stem_width=32, avg_down=True, base_width=64, cardinality=1, aa_layer=BlurPool2d,
    block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64,128}, c4_dim=256, c4_idx=set(range(0,6,2))), **kwargs)
    return _create_trans_resnet('cot_basic', pretrained, **model_args)
...
def cotnet101_hybrid_se(pretrained=False, **kwargs):
    ...
    block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64,128}, c4_dim=256, c4_idx=set(range(0,23,2))), **kwargs)
    return _create_trans_resnet('cot_basic', pretrained, **model_args)

@register_model
def cotnet101_hybrid_se_g(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 4, 23, 3],
    act_layer=get_act_layer('swish'),
    stem_type='deep', stem_width=64, avg_down=True, base_width=64, cardinality=1, aa_layer=BlurPool2d,
    block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64,128}, c4_dim=256, c4_idx=set(range(0,23,2))), **kwargs)
    model_args['global_pool'] = 'gavg'
    return _create_trans_resnet('cot_basic', pretrained, **model_args)

@register_model
def cotnet101_hybrid_se_L(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 4, 23, 3],  
    act_layer=get_act_layer('swish'),
    stem_type='deep', stem_width=64, avg_down=True, base_width=64, cardinality=1, aa_layer=BlurPool2d,
    block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64,128}, c4_dim=256, c4_idx=set(range(0,23,2))), **kwargs)
...
@register_model
def cotnet101_hybrid_se_L_g(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 4, 23, 3],  
    act_layer=get_act_layer('swish'),
    stem_type='deep', stem_width=64, avg_down=True, base_width=64, cardinality=1, aa_layer=BlurPool2d,
    block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64,128}, c4_dim=256, c4_idx=set(range(0,23,2))), **kwargs)
...
@register_model
def cotnet101_hybrid_se_XL(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 4, 23, 3],  
    act_layer=get_act_layer('swish'),
    stem_type='deep', stem_width=64, avg_down=True, base_width=64, cardinality=1, aa_layer=BlurPool2d,
    block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64,128}, c4_dim=256, c4_idx=set(range(0,23,2))), **kwargs)
...
@register_model
def cotnet152_hybrid_se_g(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 8, 36, 3],  
    act_layer=get_act_layer('swish'),
    stem_type='deep', stem_width=64, avg_down=True, base_width=64, cardinality=1, aa_layer=BlurPool2d,
    block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64,128}, c4_dim=256, c4_idx=set(range(0,36,2))), **kwargs)
...
@register_model
def cotnet152_hybrid_se_L_g(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 8, 36, 3],  
    act_layer=get_act_layer('swish'),
    stem_type='deep', stem_width=64, avg_down=True, base_width=64, cardinality=1, aa_layer=BlurPool2d,
    block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64,128}, c4_dim=256, c4_idx=set(range(0,36,2))), **kwargs)
...
@register_model
def cotnet152_hybrid_se_XL(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 8, 36, 3],  
    act_layer=get_act_layer('swish'),
    stem_type='deep', stem_width=64, avg_down=True, base_width=64, cardinality=1, aa_layer=BlurPool2d,
    block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64,128}, c4_dim=256, c4_idx=set(range(0,36,2))), **kwargs)
