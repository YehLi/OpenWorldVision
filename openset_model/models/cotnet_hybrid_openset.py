...
class BasicBlock(nn.Module):
    ...
    def __init__(self, block_idx, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
                 ...
                 attn_layer=None, aa_layer=None, drop_block=None, drop_path=None, radix=1, avd=False, avd_first=True,
                 conv_dim={},
                 c4_dim=-1, c4_idx={}):
...
class CoTBottleneck(nn.Module):
    ...
    def __init__(self, block_idx, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
        ...
        if (width in conv_dim) or (width == c4_dim and block_idx not in c4_idx):
            if stride > 1 and avd:
                self.avd = nn.AvgPool2d(3, stride, padding=1) if aa_layer is None else aa_layer(channels=width,
                                                                                                stride=stride)
                stride = 1
            ...
            if radix >= 1:
                self.conv2 = SplitAttnConv2d(
                    ...
                    drop_block=drop_block, act_layer=get_act_layer('swish'))
            else:
...
@register_model
def cotnet50_openset_hybrid_se(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 4, 6, 3],
                      act_layer=get_act_layer('swish'),
                      stem_type='deep', stem_width=32, avg_down=True, base_width=64, cardinality=1, aa_layer=BlurPool2d,
                      block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64, 128}, c4_dim=256,
                                      c4_idx=set(range(0, 6, 2))), **kwargs)
    return _create_trans_resnet('cot_basic', pretrained, **model_args)


@register_model
def cotnet101_openset_hybrid_se(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 4, 23, 3],
                      ...
                      block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64, 128}, c4_dim=256,
                                      c4_idx=set(range(0, 23, 2))), **kwargs)
    return _create_trans_resnet('cot_basic', pretrained, **model_args)

@register_model
def cotnet101_openset_hybrid_se_s(pretrained=False, **kwargs):
    model_args = dict(block=CoTBottleneck, layers=[3, 4, 23, 3],
                      act_layer=get_act_layer('swish'),
                      stem_type='deep', stem_width=64, avg_down=True, base_width=64, cardinality=1, aa_layer=BlurPool2d,
                      block_args=dict(radix=1, avd=True, avd_first=False, conv_dim={64, 128}, c4_dim=256,
