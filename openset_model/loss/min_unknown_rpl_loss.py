...
class MinUnknownRplLoss(nn.Module):
    """
    ...
    def __init__(self, gamma=0.5, lambda_o=0.1, lambda_unknown=0.1, smoothing=0.1):
        ...
        """
        super(MinUnknownRplLoss, self).__init__()
        self.gamma = gamma
    ...
    def forward(self, logits, dist_to_rp, open_dist, target):
        if not (target == 0).all():
            ...
            # L_o
            _open_dist = open_dist[(target != 0), known_target.reshape(-1), :]
            L_o = _open_dist.mean()
        else:
            L_c = logits.sum() * 0.
            L_o = open_dist.sum() * 0.
