...
def main():
    ...
    if global_rank == 0:
        ...
        saver = CheckpointSaver(
            model=bert_model, optimizer=optimizer, cfg=cfg, model_ema=model_ema, amp_scaler=loss_scaler,
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_epoch(
                epoch, model, bert_model, loader_train, optimizer, train_loss_fn, cfg,
            ...
            if cfg.distributed and cfg.BN.dist_bn in ('broadcast', 'reduce'):
                ...
                distribute_bn(bert_model, cfg.num_gpus, cfg.BN.dist_bn == 'reduce')

            logger_info('Vanilla:')
            top1_acc = evaler(epoch, model, bert_model, amp_autocast=amp_autocast)
            logger_info('EMA:')
            if model_ema is not None and not cfg.model.model_ema_force_cpu:
                if cfg.distributed and cfg.BN.dist_bn in ('broadcast', 'reduce'):
                    distribute_bn(model_ema, cfg.num_gpus, cfg.BN.dist_bn == 'reduce')
                top1_acc = evaler(epoch, model, model_ema.module, amp_autocast=amp_autocast, ema=True)
