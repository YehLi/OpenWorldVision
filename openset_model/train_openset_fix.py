...
from datasets import DatasetTxtPkl, DatasetTxt, DatasetTxtGAN, create_loader, resolve_data_config, Mixup, FastCollateMixup, AugMixDataset
...
from evaler.openset_evaler import OpensetEvaler
from evaler.tencrop_openset_evaler import TenCropOpensetEvaler
...
def setup_resume(local_rank, model, optimizer):
    ...
    if cfg.model.model_ema == True:
        ...
        if cfg.model.initial_checkpoint:
            load_checkpoint(model_ema.module, cfg.model.initial_checkpoint, use_ema=True)
        if cfg.model.resume:
...
def setup_loader(data_config):
    ...
    dataset_train = DatasetTxtPkl(
        root=cfg.data_loader.data_path,
        txt_path=cfg.data_loader.train_label,
        pkl_path=cfg.data_loader.soft_eval_label)
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        if not cfg.data_loader.prefetcher:
            input, target = input.cuda(), target.cuda().type(torch.float32)
            if mixup_fn is not None:
                input, target = mixup_fn(input, target)

        input, target = input.cuda(), target.cuda().type(torch.float32)
        off_value = cfg.loss.smoothing / cfg.model.num_classes
        val_inds = (((target - off_value).abs() < 1e-6).sum(dim=1) != (cfg.model.num_classes-1))

        with amp_autocast():
            output = model(input)
            loss = loss_fn(output, target, reduction='none')
            loss[val_inds] *= cfg.loss.soft_label_weight
            loss = loss.mean()
...
def main():
    ...
    evaler = OpensetEvaler(data_config)
    evaler_tencrop = TenCropOpensetEvaler(data_config)
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            logger_info('Vanilla:')
            top1_acc = evaler(epoch, model, amp_autocast=amp_autocast)
            logger_info('EMA:')
            if model_ema is not None and not cfg.model.model_ema_force_cpu:
                if cfg.distributed and cfg.BN.dist_bn in ('broadcast', 'reduce'):
                    distribute_bn(model_ema, cfg.num_gpus, cfg.BN.dist_bn == 'reduce')
                top1_acc = evaler(epoch, model_ema.module, amp_autocast=amp_autocast, ema=True)
                logger_info('TenCrop EMA:')
                _, _ = evaler_tencrop(epoch, model_ema.module, amp_autocast=amp_autocast, ema=True)
