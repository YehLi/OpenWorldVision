...
class OpensetEvaler(object):
    def __init__(self, data_config):
        super(OpensetEvaler, self).__init__()
    ...
    def save_results(self, epoch, loader_eval, eval_idx, ema, cls_preds_cat, indexs_cat):
        ...
        if ema:
            save_name = 'ema_pred_' + basename + '-ep{}.csv'.format(epoch)
        else:
            save_name = 'vanilla_pred_' + basename + '-ep{}.csv'.format(epoch)
        ...
        for eval_idx, loader_eval in enumerate(self.loaders_eval):
            with torch.no_grad():
                ...
                for batch_idx, (input, target, index) in enumerate(loader_eval):
                    ...
                    with amp_autocast():
                        output = model(input)
                    cls_preds.append(output.softmax(dim=1))
